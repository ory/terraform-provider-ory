...
// repo's e2e cleanup job can automatically purge stale ones.
// DO NOT CHANGE — must match the pattern in cloud/backoffice/backoffice/x/patterns.go.
...
// it can be automatically purged by the cloud repo's e2e cleanup job if not
...
// In practice, cleanup is handled by the cloud repo's e2e purge job, so failing
