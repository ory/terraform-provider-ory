...
func TestBuildPatches_CodeMaxSubmissions(t *testing.T) {
	r := &ProjectConfigResource{}
	plan := &ProjectConfigResourceModel{
		CodeMaxSubmissions: types.Int64Value(5),
	}

	patches := r.buildPatches(context.Background(), plan)
	p := findPatch(patches, "/services/identity/config/selfservice/methods/code/config/max_submissions")
	if p == nil {
		t.Fatal("expected a patch for code_max_submissions, got none")
	}
	if p.Value != int64(5) {
		t.Errorf("expected value 5, got %v", p.Value)
	}
}
