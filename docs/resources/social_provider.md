...
~> **Write-only attribute:** `auto_link` is accepted by the API on create/update but is **not returned** on read. Terraform preserves the value from state. This means drift cannot be detected from the API, and `terraform import` will not populate this field. To disable auto-linking after it has been enabled, explicitly set `auto_link = false` — simply removing the attribute from your configuration will leave auto-linking enabled server-side.
