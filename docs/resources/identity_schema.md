...
When creating a new project and setting a custom identity schema as default, provide the schema content directly in the resource configuration. There is no need to look up the schema via a data source first — the provider adds the schema to the new project's configuration, and the API automatically deduplicates if identical content already exists in the workspace.
