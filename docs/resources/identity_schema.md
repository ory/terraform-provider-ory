...
- **Automatic deduplication**: If a schema with identical content already exists in the project, the provider reuses it instead of creating a duplicate. This prevents schema accumulation when running `terraform destroy` followed by `terraform apply`.
